...
function ProblemPage({ locale, category, problem, content }: { locale: Locale; category: Content["deviceCategories"][number]; problem: Content["problems"][number]; content: Content }) {
  ...
  return (
    <main className="page-main" id="main-content">
      <JsonLd data={{ "@context": "https://schema.org", "@type": "TechArticle", headline: problem.title, description: problem.summary, inLanguage: locale, url: absoluteUrl(path), dateModified: guide?.lastReviewed ?? undefined, isBasedOn: sources.flatMap((source) => source.url ? [source.url] : []) }} />
      <div className="site-shell">
        <Breadcrumbs ariaLabel={content.messages.ui.breadcrumb} items={[{ label: content.messages.ui.home, href: paths.home(locale) }, { label: category.name, href: paths.category(locale, category) }, { label: problem.title }]} />
...
function ErrorPage({ locale, category, manufacturer, code, content }: { locale: Locale; category: Content["deviceCategories"][number]; manufacturer: Content["manufacturers"][number]; code: Content["errorCodes"][number]; content: Content }) {
  ...
  return (
    <main className="page-main" id="main-content">
      <JsonLd data={{ "@context": "https://schema.org", "@type": "TechArticle", headline: `${manufacturer.name} ${code.code}: ${code.title}`, description: code.summary, inLanguage: locale, url: absoluteUrl(paths.errorCode(locale, category, manufacturer, code)), isBasedOn: sources.flatMap((source) => source.url ? [source.url] : []) }} />
      <div className="site-shell">
        <Breadcrumbs ariaLabel={content.messages.ui.breadcrumb} items={[{ label: content.messages.ui.home, href: paths.home(locale) }, { label: category.name, href: paths.category(locale, category) }, { label: manufacturer.name, href: paths.manufacturer(locale, category, manufacturer) }, { label: code.code }]} />
